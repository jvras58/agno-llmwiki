...
class ObsidianVaultHandler(FileSystemEventHandler):
    ...
    def process_event(self, event):
        ...
        logger.info(f"Evento detectado ({event.event_type}): {os.path.basename(file_path)}")
...
def start_watcher():
    ...
    if not os.path.exists(vault_path):
        logger.error(f"O diretório {vault_path} não existe. Verifique suas variáveis de ambiente.")
        return
